        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            {#each Object.entries(stats) as [key, value]}
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    {#if loading}
                        <div class="mt-2 h-8 bg-gray-200 rounded animate-pulse"></div>
                    {:else}
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="border-b border-gray-200">
                <nav class="flex space-x-8 px-6" aria-label="Tabs">
                    <button
                        class="py-4 px-1 border-b-2 font-medium text-sm {activeTab === 'requests' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
                        on:click={() => activeTab = 'requests'}
                    >
                        Requests
                    </button>
                    <button
                        class="py-4 px-1 border-b-2 font-medium text-sm {activeTab === 'lockers' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
                        on:click={() => activeTab = 'lockers'}
                    >
                        Lockers
                    </button>
                    <button
                        class="py-4 px-1 border-b-2 font-medium text-sm {activeTab === 'users' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
