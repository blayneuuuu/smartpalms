...
function parsePublishableKey(key, options = {}) {
  ...
  if (!key || !isPublishableKey(key)) {
    ...
    if (options.fatal && !isPublishableKey(key)) {
      throw new Error("Publishable key not valid.", key);
    }
    return null;
  }
  const instanceType = key.startsWith(PUBLISHABLE_KEY_LIVE_PREFIX)
    ? "production"
    : "development";
...
function isPublishableKey(key) {
  ...
  console.log(key, hasValidPrefix, hasValidFrontendApiPostfix);
...
async function getCookieSuffix(
  ...
  return isomorphicBtoa(stringDigest)
    .replace(/\+/gi, "-")
    .replace(/\//gi, "_")
    .substring(0, 8);
}
...
var isArrayString = (s2) => {
  return (
    Array.isArray(s2) && s2.length > 0 && s2.every((a) => typeof a === "string")
  );
};
...
var validateReverificationConfig = (config) => {
  ...
  const isValidStringValue =
    typeof config === "string" && isValidVerificationType(config);
  const isValidObjectValue =
    typeof config === "object" &&
    isValidLevel(config.level) &&
    isValidMaxAge(config.afterMinutes);
...
var AccountlessApplication = class _AccountlessApplication {
  ...
  static fromJSON(data) {
    return new _AccountlessApplication(
  ...
  static fromJSON(data) {
    return new _Verification(
      ...
      data.external_verification_redirect_url
        ? new URL(data.external_verification_redirect_url)
        : null,
      data.attempts,
      data.expire_at,
      data.nonce
...
function getTraceId(data, headers) {
  if (
    data &&
    typeof data === "object" &&
    "clerk_trace_id" in data &&
    typeof data.clerk_trace_id === "string"
  ) {
...
function signedInAuthObject(authenticateContext, sessionToken, sessionClaims) {
  ...
  const getToken = createGetToken({
    ...
    sessionToken,
    fetcher: async (...args) =>
      (await apiClient.sessions.getToken(...args)).jwt,
  });
...
var AuthenticateContext = class {
  ...
  usesSuffixedCookies() {
    ...
    const suffixedSession =
      this.getSuffixedCookie(constants.Cookies.Session) || "";
  ...
  initHandshakeValues() {
    ...
    this.handshakeToken =
      ...
      this.getCookie(constants.Cookies.Handshake);
    this.handshakeRedirectLoopCounter =
      Number(this.getCookie(constants.Cookies.RedirectCount)) || 0;
  }
  ...
  getSuffixedCookie(name) {
    return (
      this.getCookie(getSuffixedCookieName(name, this.cookieSuffix)) || void 0
    );
  }
...
${error.getFullMessage()}`
  ...
  async function refreshToken(authenticateContext2) {
    ...
    const {
      ...
      refreshTokenInCookie: refreshToken2,
    ...
    try {
      const tokenResponse = await options.apiClient.sessions.refreshSession(
        decodeResult.payload.sid,
        {
  ...
  function handleMaybeHandshakeStatus(
    ...
    if (isRequestEligibleForHandshake(authenticateContext2)) {
      ...
      const isRedirectLoop =
        setHandshakeInfiniteRedirectionLoopHeaders(handshakeHeaders);
      if (isRedirectLoop) {
  ...
  function handleMaybeOrganizationSyncHandshake(authenticateContext2, auth) {
    ...
    if (organizationSyncTarget.type === "organization") {
      if (
        ...
        organizationSyncTarget.organizationSlug !== auth.orgSlug
      ) {
        mustActivate = true;
      }
      if (
        organizationSyncTarget.organizationId &&
        organizationSyncTarget.organizationId !== auth.orgId
      ) {
        mustActivate = true;
      }
    }
  ...
  function setHandshakeInfiniteRedirectionLoopHeaders(headers) {
    ...
    const newCounterValue =
      authenticateContext.handshakeRedirectLoopCounter + 1;
...
var TelemetryEventThrottler = class {
  ...
  isEventThrottled(payload) {
    ...
    const shouldInvalidate =
      entry && now - entry > __privateGet(this, _cacheTtl);
    if (shouldInvalidate) {
...
isValidBrowser_get = function () {
  ...
  try {
    ...
    const isQuotaExceededError =
      err instanceof DOMException && // Check error names for different browsers
...
var TelemetryCollector = class {
  constructor(options) {
    ...
    __privateSet(this, _config, {
      maxBufferSize:
        (_a = options.maxBufferSize) != null
          ? _a
          : DEFAULT_CONFIG.maxBufferSize,
      samplingRate:
        (_b = options.samplingRate) != null ? _b : DEFAULT_CONFIG.samplingRate,
    ...
    if (!options.clerkVersion && typeof window === "undefined") {
      ...
      __privateGet(this, _metadata).clerkVersion =
        (_e = options.clerkVersion) != null ? _e : "";
    }
    ...
    __privateGet(this, _metadata).publishableKey =
      (_f = options.publishableKey) != null ? _f : "";
  ...
  get isEnabled() {
    ...
    if (
      __privateGet(this, _config).disabled ||
      (typeof process !== "undefined" &&
        isTruthy(process.env.CLERK_TELEMETRY_DISABLED))
    ) {
      return false;
    }
    if (
  ...
  get isDebug() {
    return (
      __privateGet(this, _config).debug ||
      (typeof process !== "undefined" &&
        isTruthy(process.env.CLERK_TELEMETRY_DEBUG))
    );
  }
  record(event) {
...
shouldBeSampled_fn = function (preparedPayload, eventSamplingRate) {
  ...
  return (
    randomSeed <= __privateGet(this, _config).samplingRate &&
...
scheduleFlush_fn = function () {
  ...
  const isBufferFull =
    __privateGet(this, _buffer).length >=
    __privateGet(this, _config).maxBufferSize;
  if (isBufferFull) {
...
const DATABASE_AUTH_TOKEN =
  "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3MzYzMzU2MTcsImlkIjoiNjkyYmVhZGYtZDk1My00OGRiLTlkYjktOTBlNmNlZmMyYTI4IiwicmlkIjoiNGQ0YzhhNDMtMmFmNi00ZWY2LTlhNTQtZTg3OGEzNDBjODY0In0.DNUMVLI06kc5YysH5IV_l__5gfdvjXJQonsrqpHzZYK1_AmbfIgJAbS6-2jFS7ntEa-TedmAtTDhDQBToTrkCQ";
...
const npm_config_user_agent =
  "npm/11.0.0 node/v22.12.0 linux x64 workspaces/false";
const XDG_SEAT = "seat0";
const XDG_SESSION_TYPE = "tty";
const npm_node_execpath =
  "/home/gregoryerrl/.nvm/versions/node/v22.12.0/bin/node";
const SHLVL = "1";
...
const npm_package_json =
  "/home/gregoryerrl/Projects/web/smartpalms/package.json";
...
const WINDOWPATH = "1";
const npm_config_node_gyp =
  "/home/gregoryerrl/.nvm/versions/node/v22.12.0/lib/node_modules/npm/node_modules/node-gyp/bin/node-gyp.js";
const PATH =
  "/home/gregoryerrl/Projects/web/smartpalms/node_modules/.bin:/home/gregoryerrl/Projects/web/node_modules/.bin:/home/gregoryerrl/Projects/node_modules/.bin:/home/gregoryerrl/node_modules/.bin:/home/node_modules/.bin:/node_modules/.bin:/home/gregoryerrl/.nvm/versions/node/v22.12.0/lib/node_modules/npm/node_modules/@npmcli/run-script/lib/node-gyp-bin:/home/gregoryerrl/.nvm/versions/node/v22.12.0/bin:/usr/local/bin:/usr/bin:/bin:/usr/local/games:/usr/games";
...
const XDG_VTNR = "1";
const npm_config_globalconfig =
  "/home/gregoryerrl/.nvm/versions/node/v22.12.0/etc/npmrc";
const npm_config_init_module = "/home/gregoryerrl/.npm-init.js";
const PWD = "/home/gregoryerrl/Projects/web/smartpalms";
const npm_execpath =
  "/home/gregoryerrl/.nvm/versions/node/v22.12.0/lib/node_modules/npm/bin/npm-cli.js";
const XTERM_LOCALE = "en_PH.UTF-8";
const NVM_CD_FLAGS = "";
const XDG_DATA_DIRS =
  "/home/gregoryerrl/.local/share/flatpak/exports/share:/var/lib/flatpak/exports/share:/usr/local/share/:/usr/share/";
const npm_config_global_prefix =
  "/home/gregoryerrl/.nvm/versions/node/v22.12.0";
...
const envPrivate = /* @__PURE__ */ Object.freeze(
